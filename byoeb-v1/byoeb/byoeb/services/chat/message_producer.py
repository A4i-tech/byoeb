...
class MessageProducerService:
    ...
    def __convert_whatsapp_to_byoeb_message(
        self,
        message
    ) -> ByoebMessageContext | ByoebMessageStatus | None:
        ...
        if (
            isinstance(byoeb_message, ByoebMessageContext)
            and byoeb_message.message_context.message_type == "interactive_list_reply"
            and not byoeb_message.message_context.message_source_text
        ):
            ...
            if selected_text:
                byoeb_message.message_context.message_source_text = selected_text
    ...
    async def apublish_message(
        ...
        if byoeb_message is None:
            raise ValueError("Invalid message")

        if isinstance(byoeb_message, ByoebMessageStatus):
            # this case is already handled in QueueProducerHandler.handle() - we could assert
            # `not isinstance(byoeb_message, ByoebMessageStatus)`. Retaining it here for type
            # narrowing / static analysis.
            return "Acknowledged status message"

        assert isinstance(byoeb_message, ByoebMessageContext)
        assert byoeb_message.message_context is not None
        assert byoeb_message.user is not None
        self._logger.debug(
        ...
        mid = byoeb_message.message_context.message_id
        if mid is None:
            raise ValueError("Invalid message (no message ID specified)")
        ...
        self._message_pub_logger.info(f"Published message_id={mid} phone_number_id={byoeb_message.user.phone_number_id}", extra={AppInsightsLogHandler.DETAILS: {
            "message_id": mid,
