...
from pydantic import Field, SecretStr, TypeAdapter, field_validator, model_validator, AliasChoices
from pydantic.networks import MongoDsn
from pydantic_settings import BaseSettings, SettingsConfigDict
from typing import Literal, Optional

# MongoDsn validator — used to validate URI format without reconstructing the string.
# str(MongoDsn(v)) injects a default port, breaking mongodb+srv:// URIs.
_mongo_dsn_adapter = TypeAdapter(MongoDsn)
...
class ChatAppSettings(BaseSettings):
    ...
    mongo_db_connection_string: str = Field(
        examples=["mongodb://host:27017/mydb", "mongodb+srv://user:pass@cluster.mongodb.net/db"]
    )

    @field_validator('mongo_db_connection_string')
    @classmethod
    def validate_mongo_uri(cls, v: str) -> str:
        _mongo_dsn_adapter.validate_python(v)
        return v
