...
@pytest.mark.asyncio
async def test_registration(envs, auth_access_token, auth_session):
    auth_session.delete(f"{envs.base_url}/delete_users", json=[envs.test_user_phone_number_id]).raise_for_status()
    async with Client(f"{envs.base_url}/mcp", auth=auth_access_token) as client:
        result = await client.call_tool("asha_chat", {"message": "What is the antara injection?"})
        assert len(result.content) == 1
        assert isinstance(result.content[0], TextContent)
        ...
        assert message["phone_number_id"] == envs.test_user_phone_number_id
